...
@router.get("/current", response_model=list[weather_schema.CurrentWeatherOu])
def current(lat: Lat, lon: Lon):
    return weather_service.get_current(lat, lon)


@router.get("/daily", response_model=list[weather_schema.ForecastDayOut])
def daily(lat: Lat, lon: Lon, days: int = 5):
    return weather_service.get_daily(lat, lon, days)


@router.get("/hourly", response_model=list[weather_schema.ForecastHourOut])
